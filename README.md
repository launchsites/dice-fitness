...
A private Telegram group game: choose a player in your private controller, roll a real Telegram 🎲, and assign the exercise that face determines. Assignments stay separate and in order; they are not merged into workout totals.
...
6. In the group, an administrator replies to each participant with `/addplayer`. Each player then starts a private chat with the bot and sends `/start`.

## Game flow

The private controller remembers who each operator selected. **ROLL** picks an exercise uniformly, posts the exercise name in the group, sends Telegram's native 🎲, waits for its animation, then stores the amount dictated by that real dice face.
...
The bot ignores group commands outside `GAME_CHAT_ID`, and private controllers require an active player registration.
